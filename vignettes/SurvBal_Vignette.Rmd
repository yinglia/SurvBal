...
We specify `selection_criterion = "min_decrement_pvalue"` so that the balance that has the smallest p-value will be selected. If “min_decrement_pvalue” is used, the decrement of p-value along the forward selection procedure will be calculated, and the balance before the first decrement that is smaller than selection_threshold will be selected. The default is “min_pvalue”.
...
From the plot, we can see that the 25th percentile (lower quartile) of the sample microbiome balance is -0.0646; the 50th percentile (median) of balances is 0.1517; the 75th percentile (upper quartile) of balances is 0.4448. The survival curves for the three corresponding values of the microbiome balance are shown in the plot. 
...
Similarly, you can print the selection path, survival model, name of balance, value of balance and survival plot.

```{r}
select_coxph$selection_path
select_coxph$survival_model
select_coxph$balance_name
select_coxph$balance
select_coxph$survival_plot
```
